...
| `schema_mismatch` | 409 | Schema mismatch | A backup's database schema does not match this installation's (SPEC.md §12.6). A backup from an older schema can be migrated forward; one from a *newer* schema is refused, because this version cannot interpret data it does not know the shape of and a partial read is worse than a clear failure. |
