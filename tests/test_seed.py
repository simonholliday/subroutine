...
def test_a_fresh_workspace_gets_a_complete_vocabulary (session: sqlalchemy.orm.Session) -> None:
	...
	**A fresh workspace gets every set, not only the first**, which is what the item-type count
	...
	(`SR#1236`), then a status and a link type at version 3 (`SR#1688`), and a milestone type with
	``includes`` and ``precedes`` at version 4 (`SR#3393`). The numbers are
	...
	assert report.item_types == 13
	assert report.link_types == 8
	...
	assert type_keys == {"task", "bug", "feature", "chore", "question", "event", "milestone"}
...
def test_a_later_version_adds_only_its_own_rows (
	...
	**Its synthetic set is version 5, and it has been 2, 3 and then 4** (`SR#1236`, `SR#1688`,
	`SR#3393`). Each of those numbers became a real one — ``event`` at 2, a status and a link
	type at 3, and a milestone with two link types at 4 — so patching that key replaced a set
	the first call above had already applied, leaving
	...
	will need moving again for the same reason.** That has now happened three times, which is the
	...
	**Its synthetic link type used to be ``supersedes``**, which is a real one since `SR#1688`,
	...
	monkeypatch.setattr(subroutine.db.seed, "SEED_VERSION", 5)
