...
- **A username cannot be `set`, `unset` or `me`.** A search reads each of them as something
  other than a person - *assigned to anybody*, *to nobody*, and *to whoever is asking* - so an
  account called one could not be found by it, and a board drew a person called *unset* under
  **Nobody**.

- **`-` reads what is piped everywhere a command takes words.** `subroutine comment 42 -`
  stored a single hyphen and reported the comment as noted, where `doc create`, `doc edit`,
  `add` and `update` read the pipe. `--description` on `project create`, `project update`,
  `workspace create` and `workspace update` did the same, and so did `--because` on `done`,
  `skip`, `plan`, `defer` and `update`. All of them read the pipe now, and a `-` with nothing
  piped is refused rather than stored.
...
- **The journal no longer carries whole texts.** A comment is its opening, on one line - at
  most 140 characters, ending at a word unless that would keep fewer than a third of them -
  and `said_truncated` says whether there is more, which the item's comments still have. A
  change to a title, a description or a document's body says that it changed, with nothing
  before or after - the item's row shows its title now. `/v1/journal`, `subroutine journal`
  and the agent tools all read this way, and `/v1/changes` and an item's history still carry
  every text whole. Most of what the journal sent was documents rewritten, so a page of it is
  a small fraction of the size.
