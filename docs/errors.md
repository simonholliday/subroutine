...
| `in_use` | 409 | Still in use | The thing being removed is still referenced - a status some tasks are in, a link type some links use - or a link type is being moved into or out of a category whose rules its links were made under. The message says how many, so the caller can move them rather than guess. Removing a *tag* is deliberately not this: taking a label off the things it is on is what deleting a label means. |
...
A change-feed cursor names a point older than the events this instance still holds, so the gap between there and now cannot be reported. The client resyncs from the beginning rather than being handed a page that silently omits everything pruned in between.
...
Something with that identifying value is already here - a project key, a username, a tag name, or a document that has already been superseded.
...
The thing being removed is still referenced - a status some tasks are in, a link type some links use - or a link type is being moved into or out of a category whose rules its links were made under. The message says how many, so the caller can move them rather than guess. Removing a *tag* is deliberately not this: taking a label off the things it is on is what deleting a label means.
...
There is no such thing, or it is not visible to this caller. The two are deliberately not distinguished: saying 'forbidden' about a private project would confirm it exists.
...
The request carried a field or query parameter this endpoint does not accept. Rejected rather than ignored, because silently dropping a typo is how a caller comes to believe it set something it did not.
