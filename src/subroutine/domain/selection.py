...
def document (
	...
	**In the domain since `#2173`, and it was ``api/documents._resolve``** — the twin of
	:func:`task` above, moved for the same reason: the filter registry compiles ``parent`` and
	``under`` on a document too, has to refuse an unreachable ref by *name*, and may not import
	``api``. The endpoint keeps a delegate under the old name.
	"""
