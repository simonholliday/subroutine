...
export const MARK_ICONS = {
	...
	/* **Who the name on a row belongs to** (`#1421`, design `#1422`). Not a *state* — an
	   ...
	   `#1019`'s families are what stop a boxed `@laurence` reading as the same kind of thing as a
	...
	if (label) {
		address.push({
			...
			/* **No sigil, deliberately** (`#1019`, Simon). A project is the only mark that is
			   an address, and `#ops` and `@laurence` beside it carry theirs — so a bare word is
