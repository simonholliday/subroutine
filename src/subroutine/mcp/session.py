...
def _instructions (
	...
	**And it names the instances it is not reaching** (`#276`). Naming only the bound one is
	...
	return (
		...
		f"Read subroutine://conventions before your first write: it is what this workspace has "
		f"decided and what it has closed off, and it binds you — "
		f"/v1/documents?type=decision&status=active and /v1/documents?type=dead_end&status=active "
		f"are the same two questions for a client without resources. "
