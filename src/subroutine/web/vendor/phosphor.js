/*
	Phosphor Icons, regular weight — the path data only, extracted from
	`@phosphor-icons/core` 2.1.1 (`#764`).

	**Path data rather than the SVG files**, because every one of these is a single `<path>` in a
	`0 0 256 256` viewBox with `fill="currentColor"` — so the wrapper is identical for all of them
	...
	**Only what is used.** The set ships 1,512 icons per weight and this is twenty: an item
	...
	and a line here — `subroutine.web.vendored` records where from.
	...
	**`question` replaced `flask` on 2026-09-10** (`#2406`). The flask was the picture of
	`spike` — which is what a timeboxed experiment looks like — and `#2391` renamed that type
	to `question` and left the glyph where it was, so the word and the picture disagreed on
	every row. `#764`'s requirement is *conventional* iconography, which is what chose the
	enclosed mark over the bare `question-mark` beside it in the tarball. Nothing else drew the
	flask, so it went with the swap and the count is unchanged.

	MIT, and `phosphor.LICENSE` beside this file is the notice travelling with the code.
*/

export const PATHS = {
	/* **The two a control wears** (`#1046`, design `#1045`). Everything above says what an
	   *item* is; these say what pressing something does — which is the distinction that
	   document draws, arriving in the icon set. */
...
//: with a different grid cannot be added without this line disagreeing loudly.
